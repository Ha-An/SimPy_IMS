...
class Procurement:
    ...
    def order_material(self, supplier, inventory, daily_events, lead_time_dict):
        ...
        while True:
            ...
            if SSPOLICY:
                if inventory.on_hand_inventory + inventory.in_transition_inventory <= REORDER_QTY:
                     I[self.item_id]["LOT_SIZE_ORDER"] = ORDER_QTY[self.item_id-1] - inventory.on_hand_inventory + inventory.in_transition_inventory
                     order_size = I[self.item_id]["LOT_SIZE_ORDER"]
                else:
                    order_size = 0
            else:
                I[self.item_id]["LOT_SIZE_ORDER"] = ORDER_QTY[self.item_id-1]
